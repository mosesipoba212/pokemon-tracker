...
function generateMatchupAnalysis(a, b, sA, sB) {
  ...
  else if (sB > sA) lines.push({ type:'bad',     text:`⚠️ <b>${cap(b.name)}</b> outspeeds ${cap(a.name)} by ${sB - sA} points — ${cap(a.name)} will be attacked first.` });
  else              lines.push({ type:'neutral',  text:`⚖️ Both tie at ${sA} Speed — priority moves or natures decide order.` });
  ...
  if (aSuperEff >= 2)
    ...
    lines.push({ type:'bad',     text:`⚠️ <b>${cap(a.name)}</b>'s STAB is resisted by ${cap(b.name)} — relies on coverage moves.` });
  else
    lines.push({ type:'neutral', text:`➡️ <b>${cap(a.name)}</b>'s STAB lands neutrally against ${cap(b.name)}.` });

  if (bSuperEff >= 2)
    lines.push({ type:'bad',     text:`⚠️ <b>${cap(b.name)}</b>'s STAB type${b.types.length>1?'s':''} (${b.types.join('/')}) hit${bSuperEff===4?' for 4×':' super-effectively'} against ${cap(a.name)}.` });
  else if (bSuperEff === 0)
    ...
    lines.push({ type:'neutral', text:`➡️ <b>${cap(b.name)}</b>'s STAB lands neutrally against ${cap(a.name)}.` });
  ...
  if (aOffPhys && b.stats.def < b.stats.spd)
    ...
    lines.push({ type:'bad',     text:`⚠️ <b>${cap(b.name)}</b> has high physical Defense (${b.stats.def}) — ${cap(a.name)}'s physical attacks are less effective.` });

  const bOffPhys = b.stats.atk >= b.stats.spa;
  if (bOffPhys && a.stats.def < a.stats.spd)
    lines.push({ type:'bad',     text:`⚠️ <b>${cap(b.name)}</b> is a physical attacker vs ${cap(a.name)}'s weaker Defense (${a.stats.def}) — vulnerable.` });
  else if (!bOffPhys && a.stats.spd < a.stats.def)
    lines.push({ type:'bad',     text:`⚠️ <b>${cap(b.name)}</b> is a special attacker vs ${cap(a.name)}'s lower Sp.Def (${a.stats.spd}) — vulnerable.` });
  ...
  if (bulkA > bulkB + 60)
    ...
    lines.push({ type:'bad',     text:`⚠️ <b>${cap(b.name)}</b> is significantly bulkier (${bulkB} vs ${bulkA}) — ${cap(a.name)} may need two hits.` });
